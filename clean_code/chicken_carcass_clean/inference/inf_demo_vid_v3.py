...
def perform_inference_and_draw_boxes(model, img, output_frame, color, part_name, ii, conf_threshold=0.3, show_score=False, mask_carcass = None):
    results = model.predict(img, conf=conf_threshold, imgsz=512)
    part_count = 0
...
def FEATHER_perform_inference_and_draw_boxes(img, output_frame, mask_carcass):
    ...
    feather_results = models['feather'].predict(img, conf=part_config['feather']['conf_threshold'], imgsz=512)
    feather_on_skin_results = models['feather on skin'].predict(img, conf=part_config['feather on skin']['conf_threshold'], imgsz=512)
...
def process_video(model, video_path, output_path, area):
    ...
    while cap.isOpened():
        ...
        results = model.predict(cropped_frame, conf=0.8, imgsz=256)
        ...
        for i, (box, score, class_) in enumerate(zip(results[0].boxes.xyxy, results[0].boxes.conf, results[0].boxes.cls)):
            ...
            if (box_x1 >= 5 and box_x2 <= x2-x-5): # Only need to check 2left n right vertical line 
                ...
                # resize cropped_square to 512x512
                cropped_square = cv2.resize(cropped_square, (512, 512), interpolation=cv2.INTER_AREA)
                ...
                # resize draw_frame back to original size
                draw_frame = cv2.resize(draw_frame, (square_x2-square_x1, square_y2-square_y1), interpolation=cv2.INTER_AREA)
...
# det model
model_path = '/home/tqsang/JSON2YOLO/carcass_brio_YOLO_det_only/runs/detect/yolov8x_brio_det_only/weights/best.engine'
model = YOLO(model_path)

# Define the paths to the YOLO models and confidence thresholds for each part
part_config = {
    'feather': {'model_path': '/home/tqsang/JSON2YOLO/feather_YOLO_det/runs/detect/yolov8x_dot3_noRot/weights/best.engine', 'conf_threshold': 0.15},
    'feather on skin': {'model_path': '/home/tqsang/JSON2YOLO/feather_on_skin_YOLO_det/runs/detect/yolov8x_feather_on_skin/weights/best.engine', 'conf_threshold': 0.15},
    'wing': {'model_path': '/home/tqsang/JSON2YOLO/wing_YOLO_det/runs/detect/yolov8x_wing_dot3_nohardcase_noMosaic_noScale/weights/best.engine', 'conf_threshold': 0.2},
    'skin': {'model_path': '/home/tqsang/JSON2YOLO/skin_YOLO_det/runs/detect/yolov8x_skin_dot3_noMosaic_add/weights/best.engine', 'conf_threshold': 0.2},
    'flesh': {'model_path': '/home/tqsang/JSON2YOLO/flesh_YOLO_det/runs/detect/yolov8x_flesh_dot3/weights/best.engine', 'conf_threshold': 0.4}
}
